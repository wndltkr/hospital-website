...
export default function WriteDoctorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 진료과목 추가
      formDataToSend.append('treatment', JSON.stringify(selectedTreatments));
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || '의료진 추가에 실패했습니다.');
      }

      router.push('/admin/doctors');
      router.refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : '의료진 추가에 실패했습니다.');
    } finally {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">의료진 추가</h1>
